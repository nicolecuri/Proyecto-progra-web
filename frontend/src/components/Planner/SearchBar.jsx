...
export default function SearchBar({ onSelect }){
  ...
  const [flattenedExercises, setFlattenedExercises] = useState([])
  const [groups, setGroups] = useState(['Todos'])

  useEffect(() => {
    async function fetchExercises() {
      const data = await getAllExercises()
      setFlattenedExercises(data)
      const uniqueGroups = [...new Set(data.map(e => e.grupoMuscularPrincipal))]
      setGroups(['Todos', ...uniqueGroups.filter(Boolean)])
    }
    fetchExercises()
  }, [])

  const suggestions = flattenedExercises.filter((e) => {
